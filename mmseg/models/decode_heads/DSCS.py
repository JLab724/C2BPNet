...
class DeformableNeighborhoodAttention(nn.Module):
    ...
    @staticmethod
    def _get_offset_range(H_key, W_key, dtype, device):
        offset_y = 2.0 / (H_key - 1.0) if H_key > 1 else 0.0
        offset_x = 2.0 / (W_key - 1.0) if W_key > 1 else 0.0
        return torch.tensor(
            [offset_y, offset_x], dtype=dtype, device=device
        ).reshape(1, 2, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.offset_range_factor >= 0 and not self.no_off:
            offset_range = self._get_offset_range(
                Hk, Wk, dtype=offset.dtype, device=device)
