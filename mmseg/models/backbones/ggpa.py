...
class FeatureToIntensity(nn.Module):
    ...
    def __init__(self, in_channels, method='learned'):
        super().__init__()
        self.method = method
...
class GeoPriorGen_SC(nn.Module):
    ...
    def generate_1d_structural_contrast(self, intensity_grid, threshold=0.05):
        """Generate structural affinity along the last spatial axis."""
        diff = (intensity_grid[:, :, :, :, None]
                - intensity_grid[:, :, :, None, :])

        if self.use_soft_contrast:
            return torch.exp(-diff ** 2 / (2 * self.sigma ** 2 + 1e-6))
    ...
    def forward(self, HW_tuple: Tuple[int], intensity_map, split_or_not=False):
        ...
        if split_or_not:
            intensity_h = intensity_map.transpose(-2, -1)
            mask_i_h = self.generate_1d_intensity_decay(
                HW_tuple[0], HW_tuple[1], intensity_h)
            mask_i_w = self.generate_1d_intensity_decay(
                HW_tuple[1], HW_tuple[0], intensity_map)

            mask_h = self.generate_1d_decay(HW_tuple[0])
            mask_w = self.generate_1d_decay(HW_tuple[1])

            contrast_h = self.generate_1d_structural_contrast(intensity_h)
            contrast_w = self.generate_1d_structural_contrast(intensity_map)

            mask_h = (self.weight[0] * mask_h.unsqueeze(0).unsqueeze(2)
                      + self.weight[1] * mask_i_h + contrast_h)
            mask_w = (self.weight[0] * mask_w.unsqueeze(0).unsqueeze(2)
                      + self.weight[1] * mask_i_w + contrast_w)

            return (sin, cos), (mask_h, mask_w), None
        else:
...
class Full_GSA_SC(nn.Module):
    ...
    def forward(self, x, rel_pos, split_or_not=False):
        ...
        if split_or_not:
            ...
            out = (out_h + out_w) / 2
            out = out.permute(0, 2, 3, 1, 4).reshape(bsz, h, w, -1)
        else:
            qr = qr.flatten(2, 3)
            kr = kr.flatten(2, 3)
            vr = vr.flatten(2, 3)
